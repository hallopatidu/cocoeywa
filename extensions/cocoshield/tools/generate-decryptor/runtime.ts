...
const BUILD_VERSION:string = '1.0.0';
const ENABLE:boolean = true;
const PASSWORD:string = "QHRPcfc";
const SEED:number = 174;
const PERMS:number[]|null = null;
const RATIO:number = 0.1;
const SIGBYTES:Record<string, number> = {".png":1557,".jpg":719,".jpeg":721};
    ...
    export namespace assets{
        async function downloadFile(
            ...
            return await new Promise<DownloadResponseType>((resolve: (resp: DownloadResponseType) => void) => {
                ...
                const uncachedUrl = url.includes('?') ? `${url}&v=${BUILD_VERSION}` : `${url}?v=${BUILD_VERSION}`;
